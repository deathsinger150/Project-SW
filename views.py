...
import logging
from .models import Notification, User, Club, Admin, ClubBoardMember, ClubCommittee, ClubMember, ClubAchievement, Event, Session, Collaboration
...
def student_login_view(request):
    if request.method == 'POST':
        ...
        if user:
            ...
            print(f"Notifications for user {user}: {notifications}")

            # Pass the notifications to the student dashboard
            return render(request, 'student.html', {'notifications': notifications})
        else:
...
def student_dashboard(request):
    user = request.user  # Logged-in student user
...
def create_event_view(request):
    if request.method == 'POST':
        ...
        # Generate notifications for all students
        students = User.objects.filter(Users_role='Student')
        for student in students:
            Notification.objects.create(
                user=student,
                event=event,
                message=f"A new event '{event_name}' has been created!"
            )
